...
def _sum_overview(array: np.ndarray, level: int) -> np.ndarray:
    """Aggregate a 2D array by `level` using global-origin block sums."""
    if level == 1:
        return array.astype(np.float32, copy=False)

    rows, cols = array.shape
    out_rows = (rows + level - 1) // level
    out_cols = (cols + level - 1) // level

    pad_r = out_rows * level - rows
    pad_c = out_cols * level - cols
    if pad_r or pad_c:
        padded = np.pad(array, ((0, pad_r), (0, pad_c)), mode="constant", constant_values=0)
    else:
        padded = array

    reshaped = padded.reshape(out_rows, level, out_cols, level)
    return reshaped.sum(axis=(1, 3), dtype=np.float32).astype(np.float32, copy=False)


def create_weight_overviews(
    condition_file: str,
    output_dir: str,
    levels: list[int] | None = None,
    prefix: str = "cell_weight",
) -> dict[int, str]:
    """Create and write global overview levels for habitat-area weighting.

    Each output raster stores the SUM of valid base pixels per overview cell, globally
    anchored to the source raster origin.
    Returns a map `{level: filepath}`.
    """
    if levels is None or len(levels) == 0:
    ...
    with rasterio.open(condition_file) as src:
        band = src.read(1, masked=True).astype(np.float32)
        data = np.ma.getdata(band)
        mask_arr = np.ma.getmaskarray(band)
        valid = (~mask_arr) & np.isfinite(data)
        weight_base = valid.astype(np.float32)

        meta = src.meta.copy()
        meta.update(dtype="float32", count=1, nodata=np.nan)

        for level in levels:
            ov = _sum_overview(weight_base, int(level))
            ov_transform = src.transform * Affine.scale(level)

            out_path = out_dir / f"{prefix}_l{int(level)}.tif"
            ov_meta = meta.copy()
            ov_meta.update(
                width=ov.shape[1],
                height=ov.shape[0],
                transform=ov_transform,
            )

            with rasterio.open(out_path, "w", **ov_meta) as dst:
                dst.write(ov, 1)

            out_files[int(level)] = str(out_path)

    return out_files


def load_weight_overviews(
    overview_files: dict[int, str],
    levels: list[int],
    tile_row0: int,
    tile_col0: int,
    tile_shape: tuple[int, int],
) -> dict[int, np.ndarray]:
    """Load tile-aligned slices from global weight overviews into `{level: array}`."""
    levels = sorted({int(l) for l in levels})
    nrows, ncols = int(tile_shape[0]), int(tile_shape[1])
    tile_r1 = tile_row0 + nrows
    tile_c1 = tile_col0 + ncols

    weight_map: dict[int, np.ndarray] = {}
    for level in levels:
        if level not in overview_files:
            raise ValueError(f"Missing overview file for level {level}.")

        ov_row_start = tile_row0 // level
        ov_col_start = tile_col0 // level
        ov_row_end = (tile_r1 - 1) // level
        ov_col_end = (tile_c1 - 1) // level
        out_rows = ov_row_end - ov_row_start + 1
        out_cols = ov_col_end - ov_col_start + 1

        win = Window(
            col_off=ov_col_start,
            row_off=ov_row_start,
            width=out_cols,
            height=out_rows,
        )

        with rasterio.open(overview_files[level]) as src:
            arr = src.read(
                1,
                window=win,
                masked=True,
        ...
        arr = np.where(np.ma.getmaskarray(arr), np.nan, np.ma.getdata(arr)).astype(np.float32)
        weight_map[int(level)] = arr
